...
#anikerok change for coloring char text
func change_character_textcolor(charac):
	if charac:
		for text_node in get_tree().get_nodes_in_group('dialogic_dialog_text'):
			text_node.self_modulate = charac.color
		for text_frame in get_tree().get_nodes_in_group('dialogic_text_frame'):
			text_frame.self_modulate = charac.color
...
func update_name_label(character:DialogicCharacter):
	...
	if character_path != current_character_path:
		...
		#anikerok change
		change_character_textcolor(character)
